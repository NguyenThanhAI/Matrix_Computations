...
\subsection{Không gian con Krylov}
...
Alexei Krylov là nhà toán học người Nga đã xuất bản một công trình nghiên cứu \cite{krylov1931numerical} đề xuất một không gian con vào năm 1931. Cơ sở của không gian con này dựa trên hệ quả của định lý Cayley-Hamilton, nghịch đảo của ma trận $A$ được biểu diễn thành tổ hợp tuyến tính của các thành phần lũy thừa của ma trận $A$.
...
Bổ đề cho ta thấy nếu phổ của ma trận $A$ là $\Lambda(A)$ nằm trong một chiếc đĩa bán kính nhỏ, nằm xa gốc tọa độ sẽ đảm bảo tốc độ hội tụ nhanh của phương pháp GMRES.
...
\begin{itemize}
    \item Nếu ma trận $A$ có $n$ giá trị riêng phân biệt thì các phương pháp sẽ tìm được nghiệm chính xác sau $n$ bước.
    \item Nếu ma trận $A$ có 1 giá trị riêng phân biệt duy nhất (và khác 0) thì sau một bước lặp sẽ tìm được nghiệm chính xác.
    \item Nếu ma trận $A$ có các giá trị riêng phân thành một số ít nhóm (càng ít nhóm càng tốt), mỗi nhóm các giá trị riêng nằm sát nhau thì phương pháp cũng chỉ cần số bước lặp khoảng bằng số nhóm để đạt điều kiện hội tụ.
    \item Quá trình hội tụ sẽ khó hơn nếu ma trận $A$ có giá trị riêng nào đó nằm gần $0$ do $p_n \in P_n$ nên $p_n(0)=1$, sẽ khó khăn trong việc chọn một đa thức sao cho $p_n(0)=1$ và giá trị của nó tại một điểm gần 0 bằng 0 hoặc đủ nhỏ.
\end{itemize}

Để cải thiện đặc tính hội tụ của một hệ phương trình, ta có thể sử dụng các phương pháp điều kiện đầu (preconditioning) để cải thiện cấu trúc các giá trị riêng của ma trận $A$ như đẩy các giá trị riêng lại gần nhau hoặc đẩy các giá trị riêng gần 0 ra xa 0.

Một hình ảnh trực quan so sánh tốc độ hội tụ của hai phương pháp GMRES và phương pháp MINRES cùng áp dụng cho một hệ phương trình $Ax=b, A \in \mathbb{R}^{24 \times 24}$, $A$ là ma trận đối xứng xác định dấu dương được thực hiện trong \cite{driscoll1998potential} được biểu diễn trên hình \ref{fig:GMRES-MINRES-Convergence-Comparison}:


\begin{figure}[h!] \centering

    \includegraphics[scale=0.6]{GMRES-MINRES-Convergence-Comparison.jpg}
    \caption{So sánh tốc độ hội tụ của phương pháp GMRES và phương pháp MINRES sử dụng cho hệ phương trình $Ax=b, A \in \mathbb{R}^{24 \times 24}$, $A$ là ma trận xác định dấu dương \cite{driscoll1998potential}}

    \label{fig:GMRES-MINRES-Convergence-Comparison}
\end{figure}
